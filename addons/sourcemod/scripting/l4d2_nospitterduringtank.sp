...
{
	if (!g_bIsTankAlive) // Just in case there are multiple tanks.
	{
		g_iOldSpitterLimit = GetConVarInt(g_hSpitterLimit);
		g_iOldBoomerLimit = GetConVarInt(g_hBoomerLimit);
...
public PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_bIsTankAlive && (class == 4 || class == 2)) {
		SetConVarInt(g_hBoomerLimit, g_iOldBoomerLimit);
	} else if (class == 8) {
