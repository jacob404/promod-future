...
/**
 * Resets any variables we set during the tank.
 */
...
public Action:L4D_OnTryOfferingTankBot(tank_index, &bool:enterStatis)
{
    ...
    if (! IsFakeClient(tank_index))
    {
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
            ...
            // This should display whether or not hLightTank is enabled, but it shouldn't print twice.
            if (tankPass < 3) {
                PrintHintText(i, "Rage Meter Refilled");
                CPrintToChat(i, "{olive}[Tank Control] {default}(%N) {olive}Rage Meter Refilled", tank_index);
            } else {
                PrintHintText(i, "Final pass! Commit!");
                CPrintToChat(i, "{olive}[Tank Control] {default}(%N) {olive}Entered \"Burning Passion\" third pass", tank_index);
                if (GetEntProp(i, Prop_Send, "m_zombieClass") == 8) IgniteTank(i);
            }
        }
...
public IgniteTank(tank) {
    ...
    IgniteEntity(tank, 1000000.0);
}

public L4D2_OnWaterMove(client) {
    if (tankEnteredWater) return;
    if (!IsClientInGame(client)) return;
    if (!GetConVarBool(hLightTank)) return;
    if (tankPass < 3) return;
    if (GetEntityFlags(client) & FL_INWATER != FL_INWATER) return;
    if (GetEntProp(client, Prop_Send, "m_zombieClass") != 8) return;
    if (GetConVarBool(hTankDebug)) {
        PrintToConsoleAll("[TC] Tank entered water, starting backup damage loop.");
    }
...
public Action:BurnTank(Handle:timer, any:tank) {
    ...
    // Valve are silly, and multiply tank health by 1.5 in versus.
    new Float:dps = GetConVarFloat(FindConVar("z_tank_health"))*0.15/GetConVarFloat(hLightTankDuration);
    SDKHooks_TakeDamage(tank, 0, 0, dps, 8); // Damage is being dealt by 0, nobody, and of type 8, base_fire.
    return Plugin_Continue;
}
